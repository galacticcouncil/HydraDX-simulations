...
def value_assets(state: dict, assets: dict, prices: list) -> float:
...
def withdraw_all_liquidity(state: dict, agent_d: dict, agent_id: string) -> tuple:
    agents_d = {agent_id: agent_d}

    transaction = {
        'token_remove': 'HDX',
        'agent_id': agent_id,
        'shares_remove': -agent_d['h']
    }

    new_state, new_agents = remove_liquidity(state, agents_d, transaction)
...
def value_holdings(state: dict, agent_d: dict, agent_id: string) -> float:
    ...
    return value_assets(new_state, new_agents[agent_id], state['P'])
