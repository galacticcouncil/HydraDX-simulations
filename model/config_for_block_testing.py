...
################################# NEEDS ATTENTION ###############################################################
### a - may need to loop over in next block
### C - comes
    initial_state['Y'] = (initial_values['Ci'] * initial_state['UNI_Ri']**(-temp_a) + initial_values['Cj'] * initial_state['UNI_Rj']**(-temp_a) )**(-1/temp_a)
...
for i in configs:
    ...
    config_init_state['H'] = Omni_P_RQi * config_init_state['pool'].pool['i']['R'] + Omni_P_RQj * config_init_state['pool'].pool['j']['R']
