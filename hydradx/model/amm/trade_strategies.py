...
def price_sensitive_trading(
    ...
    class Strategy:
        ...
        def execute(self, state: GlobalState, agent_id: str) -> GlobalState:
            self.iterations += trade_frequency
            while self.iterations >= 1:
                ...
                trade_volume = max(
                    max_volume_usd / state.external_market[sell] * max(
                        max(min(1 - price_sensitivity * slip_rate, 1), 0) ** 10,
                        (0 if slip_rate > 0 else 1) * (1 - slip_rate * price_sensitivity * 50)
                    ), 0
                ) / trade_frequency
                oamm.execute_swap(
                    state=pool,
    ...
    return TradeStrategy(
        strategy_function=Strategy().execute,
        name=f'price sensitive trading (volume = {max_volume_usd} sensitivity = {price_sensitivity})'
    )
