...
def manipulate_and_withdraw(
        pool_id: str, asset1: str, asset2: str
):
        ...
        def execute(self, state: GlobalState, agent_id: str) -> GlobalState:
            ...
            if not self.invested:
                oamm.execute_swap(
                    state=omnipool,
                    agent=agent,
                    tkn_buy=self.attack_asset,
                    tkn_sell=self.trade_asset,
                    sell_quantity=agent.holdings[self.trade_asset]
                )
                self.invested = True
            elif agent.holdings[(omnipool.unique_id, self.attack_asset)] > 0:
                oamm.execute_remove_liquidity(
                    state=omnipool,
                    agent=agent,
                    quantity=agent.holdings[(omnipool.unique_id, self.attack_asset)],
                    tkn_remove=self.attack_asset
                )
            elif not self.sold_out:
                tkn_buy = self.trade_asset
                tkn_sell = self.attack_asset
                delta_r = (math.sqrt((
                    omnipool.lrna[tkn_sell] * omnipool.lrna[tkn_sell]
                    * omnipool.liquidity[tkn_sell] * omnipool.liquidity[tkn_sell]
                ) / (state.external_market[tkn_buy] / state.external_market[tkn_sell])) - (
                    omnipool.lrna[tkn_sell] * omnipool.liquidity[tkn_sell]
                )) / (omnipool.lrna[tkn_sell] + omnipool.lrna[tkn_sell])
                oamm.execute_swap(
                    state=omnipool,
                    agent=agent,
                    tkn_sell=self.attack_asset, tkn_buy=self.trade_asset,
                    sell_quantity=min(delta_r, agent.holdings[self.attack_asset])
                )
                self.sold_out = True
    ...
    return TradeStrategy(
        strategy_function=WithdrawStrategy().execute,
        name='withdraw',
    )