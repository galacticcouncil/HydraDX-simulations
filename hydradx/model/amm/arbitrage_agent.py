...
def get_arb_opps(op_state, order_book, cex_fee, buffer):
    arb_opps = []

    for arb_pair in buffer:
    # for tkn_pair in order_book_map:
        tkn_pair = arb_pair[0]
        ob_tkn_pair = arb_pair[1]
        ...
        if len(pair_order_book.bids) > 0:
            ...
            cex_sell_price = bid_price * (1 - cex_fee - buffer[arb_pair])
            ...
            if dex_buy_price < cex_sell_price:  # buy from DEX, sell to CEX
                arb_opps.append(((cex_sell_price - dex_buy_price) / dex_buy_price, arb_pair))

        if len(pair_order_book.asks) > 0:
            ask_price = pair_order_book.asks[0][0]
            cex_buy_price = ask_price * (1 + cex_fee + buffer[arb_pair])
            ...
            if dex_sell_price > cex_buy_price:  # buy from CEX, sell to DEX
                arb_opps.append(((dex_sell_price - cex_buy_price) / cex_buy_price, arb_pair))

    arb_opps.sort(key=lambda x: x[0], reverse=True)
    return arb_opps


def get_arb_swaps(op_state, cex, buffer, max_liquidity={'cex': {}, 'dex': {}}, iters=20):
    ...
    arb_opps = get_arb_opps(op_state, cex.order_book, cex_fee, buffer)
    ...
    while arb_opps:
        arb_pair = arb_opps[0][1]
        swap = process_next_swap(state, test_agent, test_cex, arb_pair[0], arb_pair[1], buffer[arb_pair],
                                 max_liquidity, iters)
        if swap:
            all_swaps.append(swap)
        else:
            break
        new_arb_opps = get_arb_opps(state, order_book, cex_fee, buffer)
...
def get_arb_swaps_simple(op_state, cex, buffer, max_liquidity={'cex': {}, 'dex': {}}, iters=20):
    # if type(buffer) == float:
    #     buffer_filled = {k: buffer for k in order_book_map}
    # else:
    #     buffer_filled = {k: buffer[k] if k in buffer else 0 for k in order_book_map}
