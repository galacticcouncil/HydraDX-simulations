...
hdx_fees_history = get_fee_history(0, min_block, max_block if max_block > 0 else None)
# list of pairs in [block_id, fee] format
# need to convert to [unique_block_id, average_fee]
fees = {}
for i in range(len(hdx_fees_history)):
    block_id = hdx_fees_history[i][0]
    fee = hdx_fees_history[i][1]
...
# plot the data
fig, ax = plt.subplots()
ax.scatter(block_ids, fees, label='HDX fees', marker='x', s=1)
ax.set_title("HDX fees history")
ax.set_xlabel("Block ID")
ax.set_ylabel("HDX fees")
ax.legend()
st.pyplot(fig)
